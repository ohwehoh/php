    ...
    <main id="contents">
        ...
        <section id="blog-type" class="center">
            <?php
                ...
                $sql = "SELECT blogImgFile, blogTitle, blogAuthor, blogRegTime, blogContents, blogLike, blogModTime FROM myBlog WHERE blogID = {$blogID}";
            ...
            <div class="blog__label" style="background-image: url(../assets/img/blog/<?=$blogInfo['blogImgFile']?>)">
                ...
                <div>
                    ...
                    <span class="date"><?=date('Y-m-d', $blogInfo['blogRegTime'])?></span><br><br>
                    <span class="modify"><a href="blogModify.php?blogID=<?=$blogID?>">수정</a></span>
                    <span class="delete"><a href="blogRemove.php?blogID=<?=$blogID?>">삭제</a></span>
                </div>
                <?php
                    if($blogInfo['blogModTime']){ ?>
                        <div class="modDateWrap">
                            <span class="modiDate">(수정 : <?=date('Y-m-d', $blogInfo['blogModTime'])?>)</span><br>
                        </div>
                <?php    }
                ?>
                    ...
                    <div class="blog__left">
                        ...
                        <?php
                            ...
                            if($_SESSION['memberID'] != ""){
                                $sql = "SELECT likeID FROM blogLike WHERE memberID = {$memberID} AND blogID = {$blogID}";
                                $result = $connect -> query($sql);
    ...
    <script>
        ...
        function blogLike(){
            ...
            if(youlikeCheck == false){
                ...
                type : "POST",           
                url : "likeCheck.php",     
                data : {"blogID": blogID, "check": 1, "likeID": "likeID"},
                dataType : "json",
                ...
                type : "POST",
                url : "likeCheck.php",
                data : {"blogID": blogID, "check": 2, "likeID": "likeID"},     
                dataType : "json",
                success : function(data){ 
                    $("#like").text("좋아요 : "+data.result);
                    youlikeCheck = false;

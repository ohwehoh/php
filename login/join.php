...
<body>
    ...
    <script>
                ...
                if(youNickName == null || youNickName == ''){
                    ...
                    success : function(data){
                        if(data.result == "good"){
                            ...
                            $("#youNickNameComment").text("이미 존재하는 닉네임입니다. 로그인을 해주세요!");
                            nickCheck = false;
                        }
                    },
                    error : function(request, status, error){
        ...
        function joinChecks(){
            ...
            //이메일 유효성 검사
            let getMail = RegExp(/^[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*@[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*.[a-zA-Z]{2,3}$/);
            if(!getMail.text($("#youEmail").val())){
            ...
            //비밀번호 공백 검사
            if($("#youPass").val() == ""){
                $("#youPassComment").text("비밀번호 입력해주세요!");
                return false;
            }
            ...
            //확인 비밀번호 공백 검사
            if($("#youPassC").val() == ""){
                $("#youPassCComment").text("확인 비밀번호 입력해주세요!");
                return false;
            }

            //비밀번호가 동일한지 체크
            if($("#youPass").val() !== $("#youPassC").val()){
                $("#youPassCComment").text("비밀번호와 일치하지 않습니다.");
                $("#youPassC").val("");
                return false;
            }
